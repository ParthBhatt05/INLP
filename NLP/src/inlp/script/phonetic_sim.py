...
from src.inlp.script.indian_language_scripts import *
import numpy as np
...
def create_similarity_matrix(sim_func, slang, tlang, normalize=True):
    dim = language_info.COORDINATED_RANGE_END_INCLUSIVE - language_info.COORDINATED_RANGE_START_INCLUSIVE + 1
    sim_mat = np.zeros((dim, dim))
    for offset1 in xrange(language_info.COORDINATED_RANGE_START_INCLUSIVE, language_info.COORDINATED_RANGE_END_INCLUSIVE + 1):
        v1 = get_phonetic_feature_vector(offset_to_char(offset1, slang), slang)
        for offset2 in xrange(language_info.COORDINATED_RANGE_START_INCLUSIVE, language_info.COORDINATED_RANGE_END_INCLUSIVE + 1):
